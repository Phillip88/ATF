    ...
    Y.codecola = Y.Base.create('codecola', Y.Widget, [], {
        ...
        _renderPanel: function(){
            ...
            Y.one('html').append(Y.Node.create(
               '<div id="codeCola" class="codeCola-wrap">'+
			    '	<h2>' + _this.chromeGetMSG("app_name") + '</h2>' +
        ...
        _bindInspect: function(){
            ...
            document.body.addEventListener('click', function(e) {
				...
				// TODO pb
				//alert(NODE_tempNode.get('id'));
				// TODO end
				
                //for Mac command key: e.metaKey
                if (e.ctrlKey == 1 || e.metaKey) {
                    ...
                    mutilNodes.push(NODE_tempNode);
					// append the properties at below ( create the panel).
					//_this.initTab1(mutilNodes, NODE_tempNode.get('nodeName'));
                    //_this.initTab2(mutilNodes, NODE_tempNode.get('nodeName'), NODE_tempNode.get('id')); // get the type of element, e.g : Input, Button, IMG...
					_this.initTab2(NODE_tempNode.get('nodeName'), NODE_tempNode.get('id'), NODE_tempNode.get('value'));
		...
		initTab2: function(type, id, value){
			var _this = this,
				wrap = Y.one('#codeCola-controls'),
				elementId = wrap.one('#elementId'),
				elementType = document.getElementById('elementType'),
				elementValue = wrap.one('#elementValue'),
				li = Y.one('#codeCola-item-fontFamily');
            ...
            wrap.set('className', '');
			if(id == ''){
				elementId.set('value', '<Non Id>');
			} else {
				elementId.set('value', id);
			}
			elementType.innerText = type;
			elementValue.set('value', value);
		},
		
		// pb-comment this function create the panel below, this panel contains all properties of selected element.
        initTab1: function(node, selector){
            var _this = this,
                wrap = Y.one('#codeCola-controls'),
		...
		// switch enabling/disabaling inspecting mode
